...
```{r}
logit.reg.synthetic <- glm(X.Synthetic.Opioids.other.than.Methadone..primarily.fentanyl..Overdose.Rate ~ ., data = train_synthetic.df, family = "binomial") 
summary(logit.reg.synthetic)
```

## Model for predicting prescribed opioid overdoses
```{r}
logit.reg.prescribed <- glm(Prescription.Opioid.Overdose.Rate ~ ., data = train_prescribed.df, family = "binomial") 

summary(logit.reg.prescribed)

```

## Prediction for synthetic opioid overdoses
```{r}
predictions_synthetic <- predict(logit.reg.synthetic, valid_synthetic.df, type = "response")
...
```{r}
predictions_prescribed <- predict(logit.reg.prescribed, valid_prescribed.df, type = "response")
